...
export async function getCreatorRevenue(creatorId: string): Promise<RevenueSummary> {
  try {
    ...
    const thisMonthTotal = thisMonthEarnings?.reduce((sum: number, e: { creator_net: string }) => sum + parseFloat(e.creator_net), 0) || 0;
    const lastMonthTotal = lastMonthEarnings?.reduce((sum: number, e: { creator_net: string }) => sum + parseFloat(e.creator_net), 0) || 0;
